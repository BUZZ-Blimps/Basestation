...
class Display:
    def __init__(self,blimpHandler):
        ...
        self.width_blimps = 175
        self.anchor_y_blimpText = 60
        self.spacing_y_blimpText = self.spacing_y_inputText
        ...
        self.anchor_x_keybind = self.width_screen - 350
        self.anchor_y_keybind = self.height_screen - 120
        self.spacing_y_keybind = 50
    ...
    def drawConnections(self):
        ...
        for i in range(0,len(blimps)):
            blimpSurface = self.getTextSurface(blimps[i].name, int(40 - len(blimps[i].name)))
            ...
            MPBPos = (self.anchor_x_MPB, self.anchor_y_MPB + i*self.spacing_y_MPB)
            MPBName = "MPB" + str(blimp.ID)
            if(not self.buttonLabelExists(MPBName)):
                self.buttons.append((MPBPos,self.MPBSurface_Enabled.get_size(),MPBName))
            if(blimp.auto == 1.0):
                self.screen.blit(self.MPBSurface_Enabled, MPBPos)
            else:
                self.screen.blit(self.MPBSurface_Disabled, MPBPos)
    ...
    def getTextSurface(self, text, size):
        textKey = text + "," + str(size)
        surface = self.textSurfaces.get(textKey)
        if(surface == None):
            surface = getTextSurface(text,size)
